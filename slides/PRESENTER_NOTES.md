...
Talking only (no demo hiccups) runs **~13 minutes** end to end — content
slides ~9 min, live demos ~4.5 min. That leaves comfortable room inside
...
| 4 | "Hear It For Yourself, Live" — websdr.org link | Content slide, no action, but the link button is real and clickable if you're presenting live with internet. Good spot for a beat of silence — let the idea land that anyone in the room can pull this up on their own phone right now. ~20-30s, or skip straight to slide 5 if you'd rather not invite a room full of phones out mid-talk. |
| 5 | "Finding a Signal in the Noise" — narrow-down/SNR/modulation bullets, Wideband Capture→Filter→Demodulate diagram, LinHT-hw link | Content slide, no action. This previews the Filter/Resample concept that shows up again on slide 9 and slide 19 — worth a quick callback later ("remember the filter-to-channel idea from earlier"). The LinHT link is a real, currently-in-development open hardware project (M17 Project); frame it as "here's someone building exactly this in hardware" rather than something available to buy today. ~35-50s. |
