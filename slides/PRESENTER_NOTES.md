...
- Antenna connected. Dummy load / attenuator on hand if you're going the
  no-radiation route for any TX demo (see slide 11's ground rules).
...
Talking only (no demo hiccups) runs **~12 minutes** end to end — content
slides ~7.5 min, live demos ~4.5 min. That leaves comfortable room inside
...
2. **Slide 18, More Demo Ideas** — point at `BRAINSTORM.md` verbally
   instead of walking the two columns.
3. **Slides 15-16, GNU Radio details / Connecting to Other Radios** —
...
4. **Slide 8, Demo 0 (FM broadcast icebreaker)** — nice-to-have proof the
   chain works, not load-bearing for the rest of the talk.
5. **Slide 14 (Demo 4, NBFM TX)** — see its row below; this is already
   framed as optional.

Do **not** cut slide 11 (Part 97 ground rules) if you're running any TX
...
| 6 | "GNU Radio + GNU Radio Companion" — toolchain diagram, GNU Radio project logo | Content slide, no action. ~30-35s. |
| 7 | "Reading a Flowgraph" — Source→Filter→Demod→Sink diagram, a real GRC screenshot | Content slide, no action. This is the concept that pays off across every demo that follows. ~25-30s. |
...
| 11 | Part 97 Ground Rules | Audience-facing legal content, read straight from the slide — no demo action here. Now sits right before the first demo that transmits (13), so say it before any RF goes out, not after. Say it **every time**, not just once, whenever a TX demo (13 or 14) is on deck. If there's no control operator in the room or you want zero on-air emission, this is where you say so and describe the dummy-load option explicitly. |
...
| 15 | "GNU Radio: What It Is, How It's Used" — history/architecture panel plus a "How to learn GNU-Radio Companion" panel with a YouTube playlist link and thumbnail | Content slide, no action. A deeper-dive follow-up to slide 6, now that the audience has seen the tool in action across five demos. The YouTube link is clickable in the actual slide file if you're presenting from the laptop and want to pull it up live; otherwise just mention it's there. ~30-40s. |
...
| 17 | "Three Kinds of Bandwidth" — SDR (RF) / Interface (USB) / Receive (decoded channel) panels | Content slide, no action. Ties together things already said earlier without naming them: slide 7's "sample rate sets your instantaneous bandwidth," the real `hackrf_info` USB-bus warning from testing this kit, and the Filter/Resample block from the flowgraph diagram. Good slide to slow down on for a more technical audience. ~40-55s. |
